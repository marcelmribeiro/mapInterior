...
function onEachFeature(f, fl) {  
    ...
    if (lg === undefined) {
      ...
      //console.log(f.properties.une_sgl_un);
      ...
      if (groupFeatBy !== '') {
        layerControl.addOverlay(mapFeatGroups[groupFeatBy],groupFeatBy,{sortLayers:true, hideSingleBase:true,collapsed:true});
        mapFeatGroups[groupFeatBy].addTo(map); // Habilita o checkbox quando é criado
        //console.log(groupFeatBy); //Mostra o Nome do que tá sendo criado
      } 
    }
...
function styleFeat(feature){
  ...
  switch(tipoFeat){    
    ...
    break;
    case 'Polygon':
      //console.log("Cor Aplicada ao ",getColor(feature));
      return { 
